...
JSON_CACHE = "/group_workspaces/jasmin2/cp4cds1/qc/qc-app/JSON_CACHE"
...
def get_all_datafile_info(url_template, ds, project, variable, table, frequency, experiment, model, ensemble,
    ...
    :return:
    """
    filename = '_'.join([variable, model, experiment, table, ensemble, version])
    json_file = os.path.join(JSON_CACHE, filename) + ".txt"
    ...
    #     json_data = jsn.load(fr)
    #
    # datafiles = json_data["response"]["docs"]
