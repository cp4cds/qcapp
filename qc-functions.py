    ...
    if not os.path.exists(log_dir):
        ...
        for file in file_list:
            if file_pattern.match(file):
                ...
                cf_global_error0 = re.compile('.*ERROR.*global.*')
                cf_global_error1 = re.compile('.*ERROR.*Convention.*')
                cf_global_error2 = re.compile('.*ERROR.*Global.*')
                cf_variable_error0 = re.compile('.*WARN(?!.*(time|boundary)).*variable.*')
                cf_variable_error1 = re.compile('.*ERROR.*units.*')
                cf_variable_error2 = re.compile('.*ERROR.*cell.*')
                cf_coordinates_error = re.compile('.*ERROR.*coordinate.*')
                cf_dimensions_error = re.compile('.*ERROR.*dimension.*')
                cf_bound_error = re.compile('.*ERROR.*bound.*')
                cf_boundary_error = re.compile('.*ERROR.*Boundary.*')
                cf_grid_error = re.compile('.*ERROR.*grid.*')
                cf_units_error = re.compile('.*ERROR.*unit.*')
